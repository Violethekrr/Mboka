        ...
        <div className="w-full max-w-sm mx-auto flex flex-col items-center">
          ...
          <form onSubmit={handleSubmit} className="w-full space-y-4">
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email ou numéro de téléphone" className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-10 py-3 text-white text-sm focus:outline-none focus:border-[#E84D4D]" />
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-3.5 text-white/20" size={18} />
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mot de passe" className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-10 py-3 text-white text-sm focus:outline-none focus:border-[#E84D4D]" />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3.5 text-white/40">
                {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
              </button>
            </div>
            <div className="text-right"><Link to="/forgot-password" className="text-[#E84D4D] text-xs hover:underline">Mot de passe oublié ?</Link></div>
            <button type="submit" className="w-full bg-[#E84D4D] hover:bg-[#c53030] text-white font-bold rounded-xl py-3 text-sm transition-all">Se connecter</button>
          </form>

          <div className="w-full text-center text-white/30 text-xs my-6">OU CONTINUER AVEC</div>
          <div className="w-full grid grid-cols-2 gap-4">
            <button className="flex items-center justify-center gap-2 bg-[#1a1a1a] border border-white/10 rounded-xl py-3 text-white text-sm hover:bg-[#252525]"><Disc size={18} /> Google</button>
            <button className="flex items-center justify-center gap-2 bg-[#1a1a1a] border border-white/10 rounded-xl py-3 text-white text-sm hover:bg-[#252525]"><Apple size={18} /> Apple</button>
          </div>
          <p className="text-center text-white/60 text-sm mt-8">Pas encore de compte ? <Link to="/inscription" className="text-[#E53E3E] font-bold">S'inscrire</Link></p>
        </div>
      </div>
    </div>
  );
