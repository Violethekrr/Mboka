...
const freelancerLinks = [
  { id: 1, label: "Accueil",    route: "/freelancer",            icon: <House size={16} />            },
...
export default function NavBar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-200 backdrop-blur-[14px] transition-all duration-300
      ${scrolled ? "bg-[#1e1e22] border-b border-[#2a2a32]" : "bg-[#1e1e22]/90 border-b border-transparent"}`}>
      <div className="max-w-350 mx-auto px-7 h-15.5 flex items-center gap-2">
        ...
        <div className="hidden md:flex gap-0.5 flex-1">
          {freelancerLinks.map(({ label, route, icon }) => {
        ...
        <div className="flex items-center gap-2 ml-auto">
                  ...
                  {notifs.map(n => (
                    <div key={n.id_notification}
                      ...
                      <div className="flex-1">
                        ...
                        <span className={`${TEXT.date} text-[#45454e]`}>{n.date}</span>
                      </div>
                      <button onClick={() => setNotifs(p => p.filter(x => x.id_notification !== n.id_notification))}
                        className="bg-none border-none text-[#6b6b76] cursor-pointer p-0 shrink-0">
                        <X size={12} />
                      </button>
                    </div>
                  ))}
          ...
          <Link to="/" title="Déconnexion"
            className="hidden md:flex w-9 h-9 rounded-lg border border-[#2a2a32] text-white/90 items-center justify-center no-underline transition-all hover:text-red-500 hover:border-red-500">
            <LogOut size={14} />
          </Link>
